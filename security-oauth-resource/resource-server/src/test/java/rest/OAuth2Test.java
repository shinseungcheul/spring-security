...
public class OAuth2Test {
	...
	public static final String OAUTH_URL = "http://localhost:8081/oauth/token";
...
//		ResponseEntity<UserEntity> res = template.exchange(REST_URL+"/user/"+userId+PARAM_ACCESS_TOKEN+token.get("access_token"), HttpMethod.GET, request, UserEntity.class);
		ResponseEntity<String> res = template.exchange(REST_URL+"/user/"+userId+PARAM_ACCESS_TOKEN+token.get("access_token"), HttpMethod.GET, request, String.class);
//		UserEntity user = res.getBody();
		String user = res.getBody();
...
//	}
	...
	public static void getAdminMain(Map<String, String> token) {
		HttpHeaders headers = getHeader();
		RestTemplate template = new RestTemplate();
		HttpEntity<String> request = new HttpEntity<>(headers);
//		ResponseEntity<UserEntity> res = template.exchange(REST_URL+"/user/"+userId+PARAM_ACCESS_TOKEN+token.get("access_token"), HttpMethod.GET, request, UserEntity.class);
		ResponseEntity<String> res = template.exchange(REST_URL+"/admin"+PARAM_ACCESS_TOKEN+token.get("access_token"), HttpMethod.GET, request, String.class);
//		UserEntity user = res.getBody();
		String user = res.getBody();
		System.out.println(user);
	}
	...
	public static void main(String[] args) {
		...
		Map<String, String> token = getOAuthToken();
...
//		removeUser(token,user.getUserId());
